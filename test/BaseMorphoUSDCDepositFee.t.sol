...
contract BaseMorphoUSDCDepositFee is Test {
    ...
    uint48 constant ClientBasisPointsOfDeposit = 9500; // 5% fee
    ...
    function test_happyPath_Morpho_with_deposit_fee() public {
        deal(USDC, clientAddress, 10000e18);

        uint256 assetBalanceBefore = IERC20(USDC).balanceOf(clientAddress);
        uint256 P2pTreasuryassetBalanceBefore = IERC20(USDC).balanceOf(P2pTreasury);
        ...
        assertEq(IERC20(USDC).balanceOf(proxyAddress), 0);
